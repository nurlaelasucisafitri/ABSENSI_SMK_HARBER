...
const KEHADIRAN_BADGE = {
  1: { color: 'hadir', text: 'Hadir' },
  2: { color: 'sakit', text: 'Sakit' },
  3: { color: 'izin', text: 'Izin' },
  4: { color: 'alfa', text: 'Tanpa keterangan' },
};
...
export default function AbsenGuru() {
  ...
  const loadAbsen = useCallback(() => {
    setLoading(true);
    api.get('/admin/absen-guru', { params: { tanggal } })
  ...
  async function submitEdit(e) {
    ...
    setSaving(true);
    try {
      await api.post('/admin/absen-guru/edit', { id_guru: editTarget.id_guru, tanggal, ...editForm });
      toast.success(`Kehadiran ${editTarget.nama_guru} berhasil diubah.`);
      setEditTarget(null);
      loadAbsen();
    } catch (err) {
  ...
  return (
    <div>
      <div className="page-header">
        <h2>Absensi Guru</h2>
        <p>Lihat dan kelola kehadiran guru berdasarkan tanggal.</p>
      </div>

      <Card headerColor="green" title="Filter Absensi">
        <div className="form-row">
          ...
          <div className="form-group" style={{ display: 'flex', alignItems: 'flex-end' }}>
            <Button variant="success" onClick={loadAbsen}><RefreshCw size={15} style={{ marginRight: 6 }} />Refresh</Button>
          </div>
        </div>
      </Card>

      <Card title="Daftar Guru" headerColor="green">
        {loading ? (
          ...
          <div className="table-wrapper">
            <table className="ui-table">
              <thead>
                <tr><th>No.</th><th>NUPTK</th><th>Nama Guru</th><th>Kehadiran</th><th>Jam Masuk</th><th>Jam Pulang</th><th>Keterangan</th><th className="text-center">Aksi</th></tr>
              </thead>
              <tbody>
                {rows.map((r, i) => {
                  ...
                  return (
                    <tr key={r.id_guru}>
                      <td>{i + 1}</td>
                      <td>{r.nuptk}</td>
                      <td><strong>{r.nama_guru}</strong></td>
      ...
      <Modal
        ...
        title={`Ubah Kehadiran — ${editTarget?.nama_guru || ''}`}
        footer={<><Button variant="outline" onClick={() => setEditTarget(null)}>Batal</Button><Button variant="success" loading={saving} onClick={submitEdit}>Simpan</Button></>}
      >
